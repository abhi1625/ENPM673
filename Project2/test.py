...
def window_centers(image,out,window_width,window_height,margin,dst):#templ,tempr):
    ...
    for win in range(0,int(windows/2)*2-5):
...
#     l_channel = gray[:,:,1]
    ...
    if th<60:
        ...
        hls = cv2.cvtColor(median,cv2.COLOR_BGR2HLS)
        s_channel = hls[:,:,2]
        s_channel = cv2.Sobel(s_channel,cv2.CV_64F,1,0)
        s_channel = np.absolute(s_channel)
        s_channel =  np.uint8(255*s_channel/np.max(s_channel))
...
#     mask = cv2.inRange(median,np.array([0,190,80]),np.array([255,255,150]))
    ...
    blur = cv2.medianBlur(seg,9)
    return blur,combined,th
...
while(True):
    ...
    undist = cv2.undistort(frame,K,dist)
    preprocess = np.zeros_like(undist[:,:,0])
    gradx = abs_sobel_thresh(undist,orient='x',thresh_min=12,thresh_max=255)
    grady = abs_sobel_thresh(undist,orient='y',thresh_min=25,thresh_max=255)
    binary = color_threshold(undist,sthresh=(100,255),vthresh=(50,255))

    preprocess[((gradx==1)&(grady==1) | (binary == 1))]=255
...
#     src = np.array([[580,450],[160,h],[1150,h],[740,450]])
    ...
    out_final= cv2.warpPerspective(frame,H,(w,h-100))
    out= cv2.warpPerspective(preprocess,H,(w,h-100))
    # out_gray = cv2.cvtColor(out,cv2.COLOR_BGR2GRAY)
    # ret,preprocess = cv2.threshold(out_gray,100,255,cv2.THRESH_BINARY)
...
#     window_centroids = curve_centers.find_window_centroids(preprocess)
#     # Points used to draw all the left and right windows
#     l_points = np.zeros([preprocess.shape[0],preprocess.shape[1]])
#     r_points = np.zeros([preprocess.shape[0],preprocess.shape[1]])
...
#         l_mask = window_mask(window_width,window_height,preprocess,window_centroids[level][0],level)
#         r_mask = window_mask(window_width,window_height,preprocess,window_centroids[level][1],level)
...
#     warpage = np.array(cv2.merge((preprocess,preprocess,preprocess)),np.uint8) # making the original road pixels 3 color channels
#     result = cv2.addWeighted(warpage, 1, template, 0.5, 0.0) # overlay the original road image with window results
    result,leftx,rightx = window_centers(out,out,window_width=50,window_height=20,margin=25,dst=dst)

    # print(rightx)
    window_height = 20
    window_width=50
    res_yvals = np.int32(out.shape[0]-np.arange(len(leftx))*window_height)

    # left_lane = np.hstack

    # p = np.polyfit()
    yvals = np.arange(0,out.shape[0])
    # print(yvals)
    ...
    cv2.imshow('s',preprocess)
    # cv2.imshow('l',preprocess)
